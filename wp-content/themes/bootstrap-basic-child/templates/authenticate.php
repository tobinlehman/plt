...
echo "<iframe id='iframe' src='$iframe_url' style='height:750px;width:100%;border:none;'></iframe>";
...
$(document).ready(function() {
    ...
    function doStuff(){
        $.ajax({
           url: 'http://devsite.work/check-session-ajax/', // Checks whether user_id session exists or not
            type: 'GET',
            success: function(receivedData){
                if(receivedData == "true"){
                    $('#link').text('Logout'); // Changes the login button text if user_id session exist
                    $('#link').attr('href', '<?php echo "http://devsite.work/single-sign-out/?http_referer=plt"; ?>'); // Changes logout url if user_id session exist
