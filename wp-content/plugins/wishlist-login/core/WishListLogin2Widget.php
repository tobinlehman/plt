...
class WishListLogin2Widget extends WP_Widget {
    ...
    public function display_membership_details($settings) {
        ...
        if (!$settings['hide_levels']) {
            ...
            if ($clevels) {
                echo __("&raquo; Levels", "&raquo; Levels", $clevels, 'wishlist-social-login');
                echo ': ';
                if ($clevels > 1)
            ...
            echo '&raquo; <a href="' . get_bloginfo('wpurl') . '/wp-admin/profile.php">' . __('Membership Details', 'wishlist-social-login') . '</a><br />';
