...
//        add_filter('cmtt_glossary_index_tooltip_content', array(__CLASS__, 'addGlossaryIndexMerriamWebsterThesaurus'), 60, 2);
		...
		add_action( 'cmtt_3rdparty_tooltip_content', array( __CLASS__, 'addMWToTooltipContent' ), 10, 2 );
	...
	public static function maybeDisableTooltips( $result, $post, $content, $force ) {
		$frontendTooltipDisableMode = get_option( 'cmtt_frontendTooltipDisableMode', 'tooltip' );
	...
	public static function glossaryToggleTooltips( $atts = array() ) {
		...
		if ( '1' === $disableTooltip || $disableTooltipSession ) {
			...
			$label				 = CMTT_Pro::__( apply_filters( 'cmtt_tooltip_on_off_widget_enable_text', 'Enable Tooltips' ) );
		} else {
			$tooltipToggleLink	 = esc_url( add_query_arg( array( 'disable_tooltips' => 1 ) ) );
			$label				 = CMTT_Pro::__( apply_filters( 'cmtt_tooltip_on_off_widget_disable_text', 'Disable Tooltips' ) );
		}
	...
	public static function glossaryTempDisableTooltips( $args ) {
		...
		$frontendTooltipDisableMode = get_option( 'cmtt_frontendTooltipDisableMode', 'tooltip' );
		if ( 'tooltip' !== $frontendTooltipDisableMode ) {
			return $args;
		}
	...
	public static function glossaryListnavShortcode( $atts = array() ) {
		...
		foreach ( $letters as $key => $letter ) {
			...
			$listNavInsideContent .= '<a class="lnletter-' . $letter . ' ln-serv-letter' . $lastClass . $disabledClass . $selectedClass . '" href="' . $link . '">' . mb_strtoupper( str_replace( 'i', 'İ', $letter ) ) . '</a>';
		}

		$listNavInsideContent .= '<div class="ln-letter-count" style="position: absolute; top: 0px; left: 88px; width: 20px; display: none;"></div></div>';
	...
	public static function glossaryTermShortcode( $atts ) {
		extract( shortcode_atts( array( 'term' => '', 'length' => '' ), $atts ) );
		...
		if ( !empty( $posts ) ) {
			...
			if ( !empty( $length ) && is_numeric( $length ) ) {
				$termDescription = cminds_truncate( $termDescription, $length );
			}
		} else {
			return FALSE;
		}
	...
	public static function addNewIconGlossaryIndex( $postItemTitleContent, $glossary_item ) {
		if ( CMTT_Glossary_Plus::isNewGlossaryItem( $glossary_item ) ) {
			$postItemTitleContent .= '<label class="cmtt-post-format-icon cmtt-post-format-new"></label>';
		}
	...
	public static function addDynamicCSS( $dynamicCss ) {
		...
		$glossaryTermTitleColor	 = str_replace( '#', '', get_option( 'cmtt_tooltipTitleColor' ) );

		ob_start();
		?>
		#tt {<?php echo $fontFamily; ?>}

		<?php if ( !empty( $glossaryTermTitleColor ) ) : ?>
			#tt #ttcont div.glossaryItemTitle {color: #<?php echo $glossaryTermTitleColor; ?>}
		<?php endif; ?>
	...
	public static function changeTermPermalink( $permalink, $glossary_item ) {
		$custom_link = get_post_meta( $glossary_item->ID, '_glossary_custom_link', true );
	...
	public static function addCategoryClass( $additionalClass, $glossary_item ) {
		...
		if ( !empty( $terms ) ) {
			foreach ( $terms as $key => $value ) {
				$additionalClass .= ' cmtt_' . $value->name;
			}
		}
	...
	public static function addTermAdditionalClass( $additionalClass, $glossary_item ) {
		$tooltipTransparency = get_post_meta( $glossary_item->ID, '_cmtt_disable_tooltip_background', true );
	 ...
	 * @return type
	 */
	public static function addMWToTooltipContent( $tooltipContent, $glossary_item ) {
		$excludeMerriamDictionaryApi = get_post_meta( $glossary_item->ID, '_cmtt_exclude_merriam_api', true );
		...
		if ( CMTT_Mw_API::dictionary_enabled() && CMTT_Mw_API::dictionary_show_in_tooltip() && !$excludeMerriamDictionaryApi ) {
			...
			if ( ($onlyOnEmpty && empty( $glossary_item->post_content )) || !$onlyOnEmpty ) {
				$tooltipContent .= CMTT_Mw_API::get_dictionary( $glossary_item->post_title );
			}
		}

		$excludeMerriamThesaurusApi = get_post_meta( $glossary_item->ID, '_cmtt_exclude_merriam_thesaurus_api', true );
		if ( CMTT_Mw_API::thesaurus_enabled() && CMTT_Mw_API::thesaurus_show_in_tooltip() && !$excludeMerriamThesaurusApi ) {
			...
			if ( ($onlyOnEmpty && empty( $glossary_item->post_content )) || !$onlyOnEmpty ) {
				$tooltipContent .= CMTT_Mw_API::get_thesaurus( $glossary_item->post_title );
			}
		}
	...
	public static function _renderCustomRelatedArticlesMetabox( $post ) {
		...
		<div id='glossary-related-article-list'>
			<?php
			$customRelatedArticles = get_post_meta( $post->ID, '_glossary_related_article' );

			if ( !empty( $customRelatedArticles ) ) {
				foreach ( $customRelatedArticles as $key => $gc ) {
					if ( isset( $gc[ 'name' ] ) && isset( $gc[ 'url' ] ) ) {
						echo '<div id="custom-related-article-' . $key . '" class="custom-related-article">';
						echo '<input type="text" name="cmtt_related_article_name[]" style="width: 40%" id="cmtt_related_article_name" value="' . $gc[ 'name' ] . '">';
						echo '<input type="text" name="cmtt_related_article_url[]" style="width: 50%" id="cmtt_related_article_url" value="' . $gc[ 'url' ] . '">';
		...
		<a id="red-add-more-rows" href="#" onclick="jQuery.fn.add_new_replacement_row();
		        return false;">Add more rows</a>
		<p style="clear: left;">
			<span class="howto">
				<?php CMTT_Pro::_e( 'Insert Name-URL pairs that will be shown before auto-generated Related articles. Both Name and URL must be supplied.' ); ?><br/>
				<strong><?php CMTT_Pro::_e( 'Once you finish, save the post/page to store changes' ); ?></strong>
			</span>
		</p><?php
	}
	...
	public static function _renderCustomTermLinkMetabox( $post ) {
		...
		<p style="clear: left;">
			<span class="howto">
				<?php CMTT_Pro::_e( 'Insert the custom URL of the Glossary Term. This URL will be used on Glossary Index and whenever the term is highlighed instead of the link to Glossary Term Page.' ); ?><br/>
				<strong><?php CMTT_Pro::_e( 'Once you finish, "Publish" or "Update" to store changes' ); ?></strong>
			</span>
		</p>
		<?php
	}
	...
	public static function addMetaboxFields( $metaboxFields ) {
		$newMetaboxFields = array_merge( $metaboxFields, array(
			'cmtt_exclude_tooltip'					 => CMTT_Pro::__( 'Hide tooltip for this term' ),
			'cmtt_disable_related_articles_for_term' => CMTT_Pro::__( 'Disable related articles for this term' ),
			'cmtt_hide_from_index'					 => CMTT_Pro::__( 'Hide term from Glossary Index' ),
			'cmtt_exclude_google_api'				 => CMTT_Pro::__( 'Disable Google API for this term' ),
			'cmtt_exclude_merriam_api'				 => CMTT_Pro::__( 'Disable Merriam-Webster Dictionary API for this term' ),
			'cmtt_exclude_merriam_thesaurus_api'	 => CMTT_Pro::__( 'Disable Merriam-Webster Thesaurus API for this term' ) ) );
	...
	public static function saveAdditionalPostData( $post_id, $post ) {
		...
		if ( isset( $post[ 'cmtt_related_article_name' ] ) && is_array( $post[ 'cmtt_related_article_name' ] ) ) {
			...
			foreach ( $post[ 'cmtt_related_article_name' ] as $key => $ra_name ) {
				if ( $ra_name != '' && $post[ 'cmtt_related_article_url' ][ $key ] != '' ) {
					$relart = array( 'name' => $ra_name, 'url' => $post[ 'cmtt_related_article_url' ][ $key ] );
					add_post_meta( $post_id, '_glossary_related_article', $relart );
				}
			}
		}
	}
	 ...
	 * @return string
	 */
	public static function outputCategories( $shortcodeAtts ) {
		$catSelectOutput = '';
		...
		if ( !empty( $shortcodeAtts[ 'freeze_cat' ] ) && $currentCategory !== array( 'all' ) ) {
			...
			$allCategoriesLabel				 = CMTT_Pro::__( get_option( 'cmtt_glossary_AllCategoriesLabel', 'All categories' ) );
...
//									$selected = selected( true, ( is_numeric( $currentCategory ) && $currentCategory == $cat->term_id || $currentCategory == $cat->name ), false );
					...
					default:
					case '1': {
							$categoriesLabel = CMTT_Pro::__( get_option( 'cmtt_glossaryCategoriesLabel' ) );
...
//									$isCurrentCategory	 = ( is_numeric( $currentCategory ) && $currentCategory == $cat->term_id || $currentCategory == $cat->name );
	...
	public static function outputBeforeListnav( $content, $shortcodeAtts, $glossary_query ) {
		...
		$showSearchButton	 = get_option( 'cmtt_glossary_showSearch', 1 );
		...
		$clearLabel			 = CMTT_Pro::__( get_option( 'cmtt_glossary_ClearLabel', '(clear)' ) );
		...
		<div class="glossary_top_filter">
			<div class="left">
				...
				if ( $showSearchButton ) :
					...
					<input value="<?php echo $searchTerm ?>" class="glossary-search-term <?php echo $additionalClass ?>" name="glossary-search-term" id="glossary-search-term" />
					<input type="button" value="<?php echo $searchButtonLabel ?>" id="glossary-search" class="glossary-search button" />
					<a class="glossary-search-clear" title="<?php CMTT_Pro::_e( 'Clear the input' ); ?>"><?php echo $clearLabel; ?></a>
				<?php endif; ?>
				<?php echo self::outputCategories( $shortcodeAtts ); ?>
			</div>
	 ...
	 * @return string
	 */
	public static function addAbbreviationsToGlossaryIndex( $glossariIndexContentArr, $glossaryItem, $preItemTitleContent,
														 $postItemTitleContent, $shortcodeAtts ) {
	...
	public static function outputGlossaryIndexItemDesc( $glossaryItemDesc, $glossary_item, $glossaryIndexStyle,
		...
		if ( in_array( $glossaryIndexStyle, array( 'modern-table', 'classic-definition' ) ) ) {
			$glossaryItemDesc = strip_tags( $glossary_item->post_content );
			if ( empty( $glossaryItemDesc ) ) {
				$glossaryItemDesc = '&nbsp;';
			}
		} else if ( in_array( $glossaryIndexStyle, array( 'classic-excerpt' ) ) ) {
			$glossaryItemDesc = strip_tags( $glossary_item->post_excerpt );
		} else {
			$glossaryItemDesc = '';
		}
		...
		if ( !empty( $glossaryItemDesc ) ) {
			...
			if ( $glossaryDescLength && $glossaryDescLength >= 30 ) {
				$glossaryItemDesc = cminds_truncate( html_entity_decode( $glossaryItemDesc ), $glossaryDescLength, '(...)' );
			}
		}
	...
	public static function addGlossaryIndexDescRelated( $glossaryItemDesc, $glossary_item, $glossaryIndexStyle,
		...
		if ( isset( $shortcodeAtts[ 'related' ] ) && $shortcodeAtts[ 'related' ] && in_array( $glossaryIndexStyle, array( 'modern-table', 'classic-definition', 'classic-excerpt' ) ) ) {
			$disableRelatedArticlesForThisTerm	 = FALSE;
			$relatedArticlesCount				 = get_option( 'cmtt_glossary_showRelatedArticlesCount' );
			$relatedArticlesGlossaryCount		 = get_option( 'cmtt_glossary_showRelatedArticlesGlossaryCount' );
			$argRelatedCount					 = $shortcodeAtts[ 'related' ];
			$relatedArticlesCount				 = min( $relatedArticlesCount, $argRelatedCount );

			$relatedSnippet = (get_option( 'cmtt_glossary_showRelatedArticles' ) == 1 && !$disableRelatedArticlesForThisTerm) ? CMTT_Related::renderRelatedArticles( $glossary_item->ID, $relatedArticlesCount, $relatedArticlesGlossaryCount, FALSE ) : '';
	...
	public static function getListnavCounts( $shortcodeAtts ) {
		...
		$args = CMTT_Pro::$lastQueryDetails[ 'nopaging_args' ];
	...
	public static function outputListnav( $listNavInsideContent, $shortcodeAtts, $glossaryQuery ) {
		...
		foreach ( $letters as $key => $letter ) {
			...
			$listNavInsideContent .= '<a class="lnletter-' . $letter . ' ln-serv-letter' . $lastClass . $disabledClass . $selectedClass . '" data-letter-count="' . $postsCount . '" data-letter="' . $letter . '" href="' . $glossaryPageLink . '">' . mb_strtoupper( str_replace( 'i', 'İ', $letter ) ) . '</a>';
		}
	...
	public static function addGlossaryIndexDefaultAtts( $atts ) {
		$atts[ 'cat' ]				 = 'all';
		$atts[ 'disable_listnav' ]	 = false;
		$atts[ 'title_prefix' ]		 = CMTT_Pro::__( 'Glossary for:' );
	...
	public static function searchTermsWhere( $where, &$wp_query ) {
		...
		$whereArr	 = array();
		$whereArr[]	 = $wpdb->posts . '.post_title LIKE "%' . $term . '%"';
		$whereArr[]	 = $wpdb->posts . '.post_content LIKE "%' . $term . '%"';
	...
	public static function renderFlushButton( $content ) {
		$additionalContent	 = '<input type="submit" name="cmtt_flush_thirdparty" value="' . CMTT_Pro::__( 'Flush API Cache' ) . '"/>';
	...
	public static function createTaxonomies() {
		...
		$glossaryCategoriesArgs		 = array(
			'label'				 => CMTT_Pro::__( 'Tooltip Categories' ),
			'rewrite'			 => array( 'slug' => $glossaryCategoriesPermalink ),
			'hierarchical'		 => true,
			'show_ui'			 => true,
		...
		$glossaryTagsArgs		 = array(
			'label'				 => CMTT_Pro::__( 'Tooltip Tags' ),
			'rewrite'			 => array( 'slug' => $glossaryTagsPermalink ),
			'hierarchical'		 => false,
			'show_ui'			 => true,
			'show_admin_column'	 => true
		);
