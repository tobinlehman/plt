<?php
if ( !class_exists( 'CmindsProPackage' ) ) {
	...
	class CmindsProPackage {
		...
		public function __construct( $config ) {
			...
			add_action( 'cminds-' . $aboutPageKey . '-content-20', array( $this, 'displayAffiliateTab' ) );
			...
			if ( $this->getOption( 'plugin-is-pro' ) ) {
				...
				${$globalVariableName}	 = $licensingApi->isLicenseOk();
				...
				if ( !$this->getOption( 'plugin-is-addon' ) ) {
					...
					add_action( 'cminds-' . $this->getOption( 'plugin-short-slug' ) . '-license-page', array( $licensingApi, 'license_page_short' ) );
					add_action( 'cminds-' . $this->getOption( 'plugin-short-slug' ) . '-update-page', array( $licensingApi, 'update_page_short' ) );
				} else {
					add_action( 'cminds-' . $this->getOption( 'plugin-parent-short-slug' ) . '-license-page', array( $licensingApi, 'license_page_short' ) );
					add_action( 'cminds-' . $this->getOption( 'plugin-parent-short-slug' ) . '-update-page', array( $licensingApi, 'update_page_short' ) );
				}
			}
		}
...
//						'30' => 'Upgrade Tutorial',
		...
		public function showAds( $atts = array() ) {
			...
			$optionName	 = 'cminds-' . $this->getOption( 'plugin-shortslug' ) . '-ads-hidden';
