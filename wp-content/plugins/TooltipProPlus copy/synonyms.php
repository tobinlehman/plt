...
class CMTT_Synonyms {
	...
	public static function addWhereFilter( $whereArr, $term, $wp_query = null ) {
		...
		if ( !empty( $theKey ) ) {
			$whereArr[] = $theKey . '.meta_value LIKE "%' . $term . '%"';
		}
		return $whereArr;
	}
...
//			self::setAllSynonyms( array(), $viewable );
		...
		if ( $existingSynonyms ) {
			...
			if ( !empty( $existingSynonyms ) ) {
				$messages .= sprintf( 'Synonym(s): "%s" is already used for one of other terms! <br/>', implode( ',', $existingSynonyms ) );
			}
	...
	public static function getSynonyms( $id, $viewable = true ) {
		...
		if ( $viewable ) {
			$synonyms = get_post_meta( $id, self::META_KEY, true );
		} else {
			$synonyms = get_post_meta( $id, self::META_KEY_INVISIBLE, true );
		}
		if ( is_array( $synonyms ) ) {
			if ( count( $synonyms ) ) {
	...
	public static function renderSynonyms( $id, $viewable = true ) {
		...
		if ( !empty( $synonyms ) ) {
			...
			$html .= '<div class=' . self::META_KEY . '_title>' . $title . ' </div><div class=cmtt_synonyms>' . esc_attr( $synonyms ) . '</div>';
			$html .= '</div>';
			return $html;
		}
	}
