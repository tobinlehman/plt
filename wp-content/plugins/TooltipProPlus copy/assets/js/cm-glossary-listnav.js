...
( function ( $ ) {
        ...
        return this.each( function () {
            ...
            function createLettersHtml() {
                ...
                for ( var i = 0; i < letters.length; i++ ) {
                    ...
                    html.push( '<a class="ln-' + ( letters[i] === '-' ? '-' : i ) + ' lnletter-' + getLowerLetter( letters[i] ) + '" href="#">' + ( ( letters[i] === '-' ) ? '...' : letters[i].replace( 'ı', 'İ' ).replace( 'i', 'I' ).toUpperCase() ) + '</a>' );
                }
    ...
    $( document ).ready( function () {
        ...
        function glossary_filter( thiz ) {
            showLoading();
            var data, currentSearch, previousSearch, searchChanged, tile = thiz.closest( '.glossary-container' );
