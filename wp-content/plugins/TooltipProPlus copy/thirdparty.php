	...
	public static function testGoogle() {
		...
		if ( self::getApiKey() != '' ) {
			if ( $source_id != -1 && $target_id != -1 ) {
				...
				CMTT_Pro::_e( 'You have to choose both source and target languages and Save Settings!' );
			}
		} else {
			CMTT_Pro::_e( 'You have to enter the API Key and Save Settings!' );
		}
		die();
	}
...
//        }
	...
	public function dictionaryShortcode( $atts ) {
		extract( shortcode_atts( array(
			'term' => '' ), $atts ) );
	...
	public function thesaurusShortcode( $atts ) {
		extract( shortcode_atts( array(
			'term' => '' ), $atts ) );
	...
	public function testDictionary() {
		...
		if ( ($key != '' ) ) {
			...
			CMTT_Pro::_e( 'You have to enter the Dictionary API and Save Settings!' );
		}
		die();
	}
	...
	public function testThesaurus() {
		...
		if ( ($key != '' ) ) {
			...
			CMTT_Pro::_e( 'You have to enter the Thesaurus API and Save Settings!' );
		}
		die();
	}
...
//                        $returnMessage = CMTT_Pro::__('Connection error. Merriam-Webster did not respond.');
						$returnMessage = '';
...
//                $returnMessage = CMTT_Pro::__('Merriam-Webster has no Dictionary definition.');
				$returnMessage = '';
							...
							foreach ( $definitionSet as $definitionKey => $definitionValue ) {
								switch ( $definitionKey ) {
									...
									case 'syn': {
											...
											$returnHtml .= '<div class=\'mw-group break\'><div class=\'group-title\'>SYNONIMS:</div>' . $content . '</div>';
											break;
										}
									case 'rel': {
...
//                $returnMessage = CMTT_Pro::__('Merriam-Webster has no Thesaurus definition.');
				$returnMessage = '';
