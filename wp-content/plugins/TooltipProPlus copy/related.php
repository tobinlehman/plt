...
class CMTT_Related {
	...
	public static function cronAddIntervals( $schedules ) {
		...
		$schedules[ 'weekly' ]	 = array(
			'interval'	 => 604800,
			'display'	 => CMTT_Pro::__( 'Once Weekly' )
		);
		$schedules[ 'monthly' ]	 = array(
			'interval'	 => 2635200,
			'display'	 => CMTT_Pro::__( 'Once Monthly' )
		);
		return $schedules;
	}
	...
	public static function getRemainingArticlesCount() {
		...
		$result					 = sprintf( __( 'Remaining articles to crawl: %d/%d' ), $remainingItemsToCrawl, $allArticles );
	...
	public static function triggerOnSave( $post_id ) {
		...
		$postTypes	 = get_option( 'cmtt_glossary_showRelatedArticlesPostTypesArr' );
	...
	public static function getCustomRelatedArticles( $glossaryId ) {
		$results		 = array();
		$glossary_cra	 = get_post_meta( $glossaryId, '_glossary_related_article' );
	...
	public static function renderRelatedArticles( $glossaryId, $limitArticles = 5, $limitGlossary = 5, $heading = false ) {
		...
		if ( count( $articles ) > 0 ) {
			...
			$html .= '<' . $tag . ' class="cmtt_related_title cmtt_related_articles_title">' . CMTT_Pro::__( get_option( 'cmtt_glossary_showRelatedArticlesTitle' ) ) . ' </' . $tag . '>';
		...
		if ( count( $glossaryArticles ) > 0 ) {
			...
			$html .= '<' . $tag . ' class="cmtt_related_title cmtt_related_terms_title">' . CMTT_Pro::__( get_option( 'cmtt_glossary_showRelatedArticlesGlossaryTitle' ) ) . ' </' . $tag . '>';
			$html .= '<ul class="cmtt_related">';
	...
	public static function renderRelatedTerms( $terms ) {
		...
		if ( count( $terms ) > 0 ) {
			...
			$html .= '<div class="cmtt_related_title cmtt_related_linked_terms_title">' . CMTT_Pro::__( get_option( 'cmtt_glossary_showRelatedTermsTitle' ) ) . '</div>';
			...
			foreach ( $terms as $term ) {
				...
				if ( get_option( 'cmtt_glossary_relatedArticlesPrefix' ) ) {
					$title = CMTT_Pro::__( get_option( 'cmtt_glossary_relatedTermsPrefix' ) ) . ' ' . $title;
				}
