...
if( is_plugin_active('memberpress/memberpress.php') ) {
  ...
  class MpimpAppController {
    ...
    public function route() {
      ...
      if( strtoupper( $_SERVER['REQUEST_METHOD'] ) == 'POST' ) {
        ...
        if($results['status']=='complete' or !empty($errors)) { @unlink($filepath); }
            ...
            <div>
              ...
              <select id="mpimp-import-file-type">
                <?php foreach( $this->importers as $importer => $impclass ): ?>
                  <option value="<?php echo $importer; ?>"><?php echo $this->human_readable_class($impclass); ?></option>
                <?php endforeach; ?>
              </select>
            </div>
    ...
    public function import_from_csv($importer,$filepath,$args=array(),$row=0,$offset=0,$headers=array(),$cli=false) {
      ...
      if( $row==0 ) {
        if(($headers=$data=fgetcsv($fh))!==false) { $row++; }
      }
      else {
  ...
  MpimpUpdateController::load_hooks();
}
