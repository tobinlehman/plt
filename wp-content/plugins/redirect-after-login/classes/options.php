...
class MTRAL_Options {
	...
	public function callback_mtral( $args ) {
		...
		foreach($roles as $role_slug => $role_options){
			...
			$has_admin_custom_pages = (!empty($admin_custom_pages)) ? 'disabled' : '';
			...
			$html .= sprintf( '<select id="%1$s_%4$s" name="%2$s[%1$s_%4$s]"%3$s %5$s>', $id, $tab, $this->build_field_attributes( $attrs ), $role_slug, $has_admin_custom_pages );
