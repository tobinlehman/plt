...
class MeprReminder extends MeprCptModel {
  ...
  public function get_query_products($join_name) {
    if($this->filter_products && is_array($this->products) && !empty($this->products)) {
      return "AND {$join_name} IN(" . implode(',', $this->products) . ")";
    }
  ...
  public function get_next_expired_cc() {
    ...
    $prd_id_str = $wpdb->prepare('%s', MeprSubscription::$product_id_str);
    ...
    //Make sure we're only grabbing from valid product ID's for this reminder yo
    //If $this->products is empty, then we should send for all product_id's
    $and_products = $this->get_query_products('pm_prd_id.meta_value');

    // This needs to go when we move to the mepr_subscriptions table
    $and_products_join = '';
    if($this->filter_products && is_array($this->products) && !empty($this->products)) {
      $and_products_join = "INNER JOIN {$wpdb->postmeta} AS pm_prd_id
                               ON pm_prd_id.post_id = sub.ID
                              AND pm_prd_id.meta_key = {$prd_id_str} ";
    }
