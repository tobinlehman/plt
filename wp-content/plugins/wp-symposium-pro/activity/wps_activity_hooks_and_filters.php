...
add_filter( 'wp_nav_menu_items', 'wps_activity_add_username' );
function wps_activity_add_username($items){ 
    ...
    if (is_user_logged_in()):
        ...
        if (!get_option('wpspro_profile_permalinks')):
            $profile_page = get_post(get_option('wpspro_profile_page'));
            $slug = '/'.$profile_page->post_name.'/';
            $items = str_replace($slug, $slug.$current_user->user_login, $items);
        else:
            $items = str_replace($url, $url.wps_query_mark($url).'user_id='.$current_user->ID, $items);
        endif;
...
function wps_activity_post_add_alerts($the_post, $the_files, $new_id) {
	...
	if (post_type_exists('wps_alerts')):
		...
		$get_recipient = $get_the_post ? get_user_by('id', $get_the_post->wps_target) : false;
		if ($get_recipient && $get_the_post->wps_target != $the_post['wps_activity_post_author']) {
			...
			if (get_option('wpspro_all_friends_alerts')):
				// ... if so, carry on
				global $current_user;	
				$friends = wps_get_friends($current_user->ID, false);
				if ($friends):
		...
		if ($get_the_post && !$get_the_post->wps_target_type):
				...
				foreach ($recipients as $target_id):
					...
					if ( (int)$target_id != (int)$current_user->ID && !in_array($target_id, $sent) ):
						...
						if ($status == 'publish'):
							...
							update_post_meta( $new_alert_id, 'wps_alert_note', __('Chosen not to receive activity alerts by email (post) #'.get_user_meta($target_id, 'wps_activity_subscribe', true).'#', WPS2_TEXT_DOMAIN) );
						endif;
				...
				foreach ($recipients as $key=>$value):
					...
					if ($value):
						...
						if ( (int)$value != (int)$current_user->ID && !in_array($value, $sent) ):
							...
							if ($status == 'publish'):
								update_post_meta( $new_alert_id, 'wps_alert_failed_datetime', current_time('mysql', 1) );
								update_post_meta( $new_alert_id, 'wps_alert_note', __('Chosen not to receive activity alerts by email (comment) #'.get_user_meta($target_id, 'wps_activity_subscribe', true).'#', WPS2_TEXT_DOMAIN) );
							endif;
