...
function alerts_columns_head($defaults) {
    global $post;
	if ($post && $post->post_type == 'wps_alerts') {
		//$defaults['col_id'] = 'ID';
...
function alerts_columns_content($column_name, $post_ID) {
    ...
    if ($column_name == 'col_recipient') {
		...
		if ($user):
			...
			echo '<div style="color: #f00">'.sprintf(__('No recipient, <a href="%s">clean up subscriptions</a>?', WPS2_TEXT_DOMAIN), admin_url( 'edit.php?post_type=wps_alerts&wps_cleanup=1' )).'</div>';
		endif;
    }
...
function wps_alerts_clear_sent( $views )
{
	if ( current_user_can( 'manage_options' ) ):
		...
		if (isset($_REQUEST['wps_action'])):
			...
			if ( wp_verify_nonce( $nonce, 'wps_alerts_clear' ) ) {
				...
				if ($_REQUEST['wps_action'] == 'wps_alerts_clear_sent'):
					...
					echo __('Sent alerts removed (please wait, refreshing page...).', WPS2_TEXT_DOMAIN);
					echo '</p></div>';
				...
				if ($_REQUEST['wps_action'] == 'wps_alerts_clear_pending'):
					...
					echo '<div class="updated"><p>';
					echo __('Pending alerts removed (please wait, refreshing page...).', WPS2_TEXT_DOMAIN);
					echo '</p></div>';

				endif;

				if ($_REQUEST['wps_action'] == 'wps_alerts_clear_all'):

					$sql = "DELETE FROM ".$wpdb->prefix."posts WHERE post_type='wps_alerts' and (post_status = 'pending' or post_status = 'publish')";
					$wpdb->query($sql);

					echo '<div class="updated"><p>';
					echo __('All alerts removed (please wait, refreshing page...).', WPS2_TEXT_DOMAIN);
					echo '</p></div>';

				endif;

				echo '<script type="text/javascript">';
				echo 'window.location = window.location.pathname+\'?post_type=wps_alerts\';';
				echo '</script>';
        ...
        if (get_option('wps_alert_resend')) $views['wps-alerts-pending-sent'] .= ' ('.__('re-send enabled', WPS2_TEXT_DOMAIN).')';
    	$views['wps-alerts-clear-all'] = '<a onclick="return confirm(\''.__('Are you sure, this cannot be undone?', WPS2_TEXT_DOMAIN).'\')" id="wps_alerts_clear_sent" href="edit.php?post_type=wps_alerts&wps_action=wps_alerts_clear_all&_wpnonce='.$nonce.'">'.__('Remove all alerts', WPS2_TEXT_DOMAIN).'</a>';
    	if (function_exists('__wps__wpspro_extensions_au'))
    		$views['wps-alerts-cleanup'] = '<a id="wps_alerts_cleanup" href="edit.php?post_type=wps_alerts&wps_cleanup=1">'.__('Cleanup invalid subscriptions', WPS2_TEXT_DOMAIN).'</a>';    	
    	
    	$views['wps-server-time'] = __('Server time', WPS2_TEXT_DOMAIN).': '.current_time('mysql', 1);
