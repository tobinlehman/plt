...
add_action('wp_ajax_wps_add_to_site', 'wps_add_to_site');

/* ADD TO SITE (MULTI-SITE) */
function wps_add_to_site() {
...
function wps_check_login($user, $username, $password) {
    ...
    if ($user && !is_wp_error($user) && $user->ID && $username && wps_is_account_closed($user->ID)) $return = new WP_Error('wps_login_fail', __('This account is closed.', WPS2_TEXT_DOMAIN));
...
function wps_usermeta_form_save( $user_id ) {
    ...
    $home_form = strip_tags($_POST['wpspro_home']);
    $country_form = strip_tags($_POST['wpspro_country']);
