...
// Shortcodes
require_once('wps_avatar_shortcodes.php');
...
function user_avatar_core_set_avatar_constants() {
	...
	if ( !defined( 'USER_AVATAR_DEFAULT' ) )
		define( 'USER_AVATAR_DEFAULT', plugins_url('/wp-symposium-pro/avatar/images/mystery-man.jpg') );

	if ( !defined( 'USER_AVATAR_DEFAULT_THUMB' ) )
		define( 'USER_AVATAR_DEFAULT_THUMB', plugins_url('/wp-symposium-pro/avatar/images/mystery-man-50.jpg') );
...
function user_avatar_core_avatar_upload_path($uid = false)
{
	...
	if( !file_exists(WP_CONTENT_DIR."/wps-pro-content/members/".$uid."/avatar/") )
		mkdir(WP_CONTENT_DIR."/wps-pro-content/members/".$uid."/avatar/", 0777 ,true);
	return WP_CONTENT_DIR."/wps-pro-content/members/".$uid."/avatar/";
}
...
{	
	...
	return WP_CONTENT_URL."/wps-pro-content/members/".$uid."/avatar/";
}
...
<?php
	...
	// Bootstrap
	wp_enqueue_style	('bootstrap', plugins_url().'/wp-symposium-pro/bootstrap/css/bootstrap.min.css', 'css');
	wp_enqueue_style	('bootstrap-theme', plugins_url().'/wp-symposium-pro/bootstrap/css/bootstrap-theme.min.css', 'css');
	wp_enqueue_script	('bootstrap-js', plugins_url().'/wp-symposium-pro/bootstrap/js/bootstrap.min.js', array('jquery'));	
...
<?php
		...
		wp_die(__("You are not allowed to do that.", WPS2_TEXT_DOMAIN));
	}
	die();
}
...
function user_avatar_add_photo_step1($uid)
{
	...
	<form enctype="multipart/form-data" id="uploadForm" method="POST" action="<?php echo admin_url('admin-ajax.php'); ?>?action=user_avatar_add_photo&step=2&uid=<?php echo $uid; ?>" >
		<label for="upload"><?php _e('Choose an image from your computer:', WPS2_TEXT_DOMAIN); ?></label><br /><input title="Choose an image" type="file" id="upload" name="uploadedfile" />

		<?php wp_nonce_field('user-avatar') ?>
		<p class="submit"><input type="submit" class="wps_button btn btn-primary" value="<?php esc_attr_e('Upload'); ?>" /></p>
	</form>
	</div>
...
function user_avatar_add_photo_step2($uid)
{
		...
		if (!(($_FILES["uploadedfile"]["type"] == "image/gif") || ($_FILES["uploadedfile"]["type"] == "image/jpeg") || ($_FILES["uploadedfile"]["type"] == "image/png") || ($_FILES["uploadedfile"]["type"] == "image/pjpeg") || ($_FILES["uploadedfile"]["type"] == "image/x-png"))){
			echo "<div class='error'><p>".__("Please upload an image file (.jpeg, .gif, .png).", WPS2_TEXT_DOMAIN)."</p></div>";
			user_avatar_add_photo_step1($uid);
			die();
		}
        ...
        if ($file_size > 500):
            echo "<div class='error'><p>".sprintf(__("Please upload an image file less than 500KB (yours was %dKB).", WPS2_TEXT_DOMAIN), $file_size)."</p></div>";
            die();
        else:
            ...
            <input type="submit" class="wps_button btn btn-primary" style="margin-left:0;" id="user-avatar-crop-button" value="<?php esc_attr_e('Crop Image', WPS2_TEXT_DOMAIN); ?>" />
            </div>
            </form>
...
function user_avatar_add_photo_step3($uid)
{
	...
	if( !file_exists($original_file) ) {
		echo "<div class='error'><p>". __('Sorry, No file available', WPS2_TEXT_DOMAIN)."</p></div>";
		return true;
	}

	$time_now = time();

	$cropped_full = user_avatar_core_avatar_upload_path($uid).$time_now."-wpsfull.jpg";
	$cropped_thumb = user_avatar_core_avatar_upload_path($uid).$time_now."-wpsthumb.jpg";
	...
	/* Update user's meta data for quick reference */
  	update_user_meta( $uid, 'wps_pro_avatar', "/wps-pro-content/members/".$user_id."/avatar/".$time_now."-wpsfull.jpg" );
	...
	<div id="user-avatar-step3">
		<h3><?php _e("Avatar updated...", WPS2_TEXT_DOMAIN); ?></h3>
		<span style="float:left;">
		<?php
		echo user_avatar_get_avatar( $uid, 150);
		?>
		</span>
		<!-- <a id="user-avatar-step3-close" style="cursor:pointer" onclick="self.parent.tb_remove();" ><?php _e('Close', WPS2_TEXT_DOMAIN); ?></a> -->
	</div>
<?php	
}	
/**
...
function user_avatar_delete_files($uid)
{
	...
	delete_user_meta( $uid, 'wps_pro_avatar' );
...
function user_avatar_fetch_avatar( $args = '' ) {
	...
	$avatar_size = ( 'full' == $type ) ? '-wpsfull' : '-wpsthumb';
	...
	if( $avatar_img = user_avatar_avatar_exists( $item_id, $avatar_size ) ):
		...
		$avatar_url = content_url()."/wps-pro-content/members/".$item_id."/avatar/".$avatar_img;
...
function user_avatar_form($profile)
{
	...
	?>
	<h3><?php _e('Picture', WPS2_TEXT_DOMAIN); ?></h3>
	...
	<a id="user-avatar-link" class="button-secondary thickbox" href="<?php echo admin_url('admin-ajax.php'); ?>?action=user_avatar_add_photo&step=1&uid=<?php echo $profile->ID; ?>&TB_iframe=true&width=720&height=450" title="<?php _e('Upload and Crop and Image to be Displayed', WPS2_TEXT_DOMAIN); ?>" ><?php _e('Update Picture', WPS2_TEXT_DOMAIN); ?></a> 
	...
	<?php 
		...
		if ( user_avatar_avatar_exists($profile->ID) ):?>
			...
			<a id="user-avatar-remove" class="submitdelete deleteaction" href="<?php echo esc_url_raw($remove_url); ?>" title="<?php _e('Remove User Avatar Image', WPS2_TEXT_DOMAIN); ?>" ><?php _e('Remove', WPS2_TEXT_DOMAIN); ?></a>
			<?php
		endif;
	?>
	</div>
	...
	function add_remove_avatar_link(){
		if(!jQuery("#user-avatar-remove").is('a')){
			jQuery('#user-avatar-link').after(" <a href='<?php echo $remove_url; ?>' class='submitdelete'  id='user-avatar-remove' ><?php _e('Remove', WPS2_TEXT_DOMAIN); ?></a>")
		}
 ...
 * @return void
 */
function user_avatar_avatar_exists($id, $type = "-wpsfull"){
	...
	if ($id):
		...
		if( user_avatar_avatar_exists($id) ):
			...
			if($user_avatar):
				...
				$ret = '<img src="'.USER_AVATAR_DEFAULT.'" width="'.$width.'" height="'.$width.'" class="avatar" />';
			endif;
		else:
		...
		if ($link):
			if (function_exists('icl_link_to_element')):
				...
				$html = '<a href="'.$url.wps_query_mark($url).'user_id='.$id.'">';
		...
		$ret = '<img src="'.plugins_url('/wp-symposium-pro/avatar/images/mystery-man.jpg').'" width="'.$width.'" height="'.$width.'" class="avatar" />';;

	endif;

    // Filter for any actions
    $ret = apply_filters( 'wps_get_avatar_filter', $ret, $id );
