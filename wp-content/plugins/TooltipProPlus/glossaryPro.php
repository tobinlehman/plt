...
//			$value = apply_filters( 'cm_tooltip_parse', $value, true );
			$value = self::cmtt_glossary_parse( do_shortcode($value), true );
		}
		return $value;
	}
...
//		set_transient( $transientKey, $parserArrays, 60 * 60 * 24 * 30 );
	...
	public static function cmtt_glossary_parse( $content, $force = false ) {
		...
		if ( !$force ) {
			if ( !get_option( 'cmtt_glossaryEnableCaching', FALSE ) ) {
				...
				$oldContentHash = get_transient( $contentHashKey );
		...
		$glossarySearchStringArr = self::prepare_parser_string_arr();
	...
	public static function cmtt_prepareReplaceTemplate( $title ) {
		...
		if ( get_option( 'cmtt_glossaryTooltip' ) == 1 && $excludeTT != 1 ) {
			...
			if ( $removeLinksToTerms ) {
				$link_replace = '<span ' . $titleAttr . ' data-cmtooltip="' . $tooltipContent . '" class="glossaryLink ' . $additionalClass . '">' . $titlePlaceholder . '</span>';
			} else {
				$link_replace = '<a href="' . $permalink . '"' . $titleAttr . ' data-cmtooltip="' . $tooltipContent . '"  class="glossaryLink ' . $additionalClass . '"' . $windowTarget . '>' . $titlePlaceholder . '</a>';
			}
		} else {
			if ( $removeLinksToTerms ) {
	...
	public static function cmtt_custom_tooltip_shortcode( $atts, $text = '' ) {
		...
		$tooltip = '<span data-cmtooltip="' . $content . '" class="glossaryLink">' . $text . '</span>';
		return $tooltip;
	}
