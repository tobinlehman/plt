 ...
 * @version    $Id: ZendServer.php 1411129 2016-05-05 16:15:58Z worschtebrot $
 */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class IfwPsn_Vendor_Zend_Cache_Backend_ZendServer extends IfwPsn_Vendor_Zend_Cache_Backend implements IfwPsn_Vendor_Zend_Cache_Backend_Interface
{
    /**
     * Available options
     *
    ...
    public function test($id)
    {
        ...
        if ($tmp !== false) {
            if (!is_array($tmp) || !isset($tmp['mtime'])) {
                IfwPsn_Vendor_Zend_Cache::throwException('Cache metadata for \'' . $id . '\' id is corrupted' );
            }
            return $tmp['mtime'];
        }
        return false;
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        if (count($tags) > 0) {
            $this->_log('IfwPsn_Vendor_Zend_Cache_Backend_ZendServer::save() : tags are unsupported by the ZendServer backends');
        }
