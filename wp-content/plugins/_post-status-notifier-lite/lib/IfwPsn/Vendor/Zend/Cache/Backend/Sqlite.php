 ...
 * @version    $Id: Sqlite.php 1411129 2016-05-05 16:15:58Z worschtebrot $
 */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwPsn_Vendor_Zend_Cache_Backend_Sqlite extends IfwPsn_Vendor_Zend_Cache_Backend implements IfwPsn_Vendor_Zend_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        $res = $this->_query($sql);
        if (!$res) {
            $this->_log("IfwPsn_Vendor_Zend_Cache_Backend_Sqlite::save() : impossible to store the cache id=$id");
            return false;
        }
        $res = true;
        foreach ($tags as $tag) {
    ...
    private function _getConnection()
    {
        if (is_resource($this->_db)) {
            ...
            if (!(is_resource($this->_db))) {
                IfwPsn_Vendor_Zend_Cache::throwException("Impossible to open " . $this->_options['cache_db_complete_path'] . " cache DB file");
            }
            return $this->_db;
        }
    }
    ...
    private function _registerTag($id, $tag) {
        ...
        if (!$res) {
            $this->_log("IfwPsn_Vendor_Zend_Cache_Backend_Sqlite::_registerTag() : impossible to register tag=$tag on id=$id");
            return false;
        }
        return true;
    }
    ...
    private function _checkStructureVersion()
    {
        ...
        if (((int) $row['num']) != 1) {
            // old cache structure
            $this->_log('IfwPsn_Vendor_Zend_Cache_Backend_Sqlite::_checkStructureVersion() : old cache structure version detected => the cache is going to be dropped');
            return false;
        }
        return true;
    }
     ...
     * @return boolean True if no problem
     */
    private function _clean($mode = IfwPsn_Vendor_Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch ($mode) {
