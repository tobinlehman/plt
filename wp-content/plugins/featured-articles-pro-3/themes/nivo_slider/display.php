	...
	<div class="nivoSlider fa-nivo-slider">
    	<?php while( have_slides() ):?>
    		<?php 
				...
				$content = the_fa_title( '<h2>', '</h2>', false ) . the_fa_date( '<span class="fa-date"> ', '</span>', false ). the_fa_author( '<span class="fa-author">' . __(' by '), '</span> ', false ) . the_fa_content( '', '', false ) . the_fa_read_more( 'fa-read-more', false ) . the_fa_play_video( 'fa-play-video', 'modal', true, false ) . the_fa_video( '', '', false, false );
    			if( !empty( $content ) ){
    				$captions[] = sprintf(
