...
function have_slides(){
	...
	if ( $fa_slider->current_slide + 1 < $fa_slider->slide_count ) {
		...
		/**
		 * Action triggered on every slide in the loop.
		 * 
		 * @param $slide_index
		 * @param $slide_post_object
		 * @param $slider_post_object
		 */
		do_action('slides_loop_running', $fa_slider->current_slide, get_current_slide(), $fa_slider );		
...
function the_fa_image( $before = '<div class="image_container">', $after = '</div>', $set_width = false, $overlay_link = true, $echo = true ){
	...
	if( !$slider_options['show'] ){
		$video_options =  fa_get_slider_options( $fa_slider->ID, 'content_video' );
		if( $video_options['show'] ){
			the_fa_video( $before, $after, $echo );
		}		
		return;
	}
	...
	if( !$image ){
		$video_options =  fa_get_slider_options( $fa_slider->ID, 'content_video' );
		if( $video_options['show'] ){
			// add class no-image to container
			if( !empty( $before ) ){
				$before = preg_replace( '|class="(.*)"|Ui' , 'class="$1 no-image"', $before, 1, $replacements);
				if( 0 === $replacements ){
					$before = substr_replace( $before , ' class="no-image">', strpos( $before, '>' ) );
				}
			}	
			the_fa_video( $before, $after, $overlay_link, $echo );
		}		
		return false;
	}
	// the image URL
	...
	$video = the_fa_video( $before, $after, false, false );
	$video_link = '';
	if( $video && $overlay_link ){
		$video_link = sprintf('<a href="#" class="play-video image-over"></a>');
	}	
	...
	// start the output
	$img_output = $before . '<div class="' . implode(' ', $container_class) . '" ' . implode(' ', $container_data) . '>' . $link[0] . $img_html . $link[1] . $video_link . '</div>' . $video . $after;
	/**
...
/**
 * Embeds a video into a slider
 */
function the_fa_video( $before = '', $after = '', $overlay_link = false, $echo = true ){
	global $fa_slider;
	
	$video_data = get_the_fa_video();
	if( !$video_data ){
		return;
	}
	...
	// video data
	$video_attrs = array();
	foreach ( $video_data as $attr => $val ){
		$video_attrs[] = 'data-' . $attr . '="' . $val . '"';
	}
	
	$height = fa_player_height( $video_data['aspect'] , $video_data['width'] );
	$output = '<div class="fa-video-wrap"><div title="' . esc_attr( get_the_fa_title() ) . '" style="display:none; max-width:100%; width:' . $video_data['width'] . 'px; height:' . $height . 'px" class="fa-video" ' . implode(' ', $video_attrs) . '><!-- video container --></div></div>';
	if( $overlay_link ){
		$output.= '<a href="#" class="play-video image-over"></a>';
	}
...
/**
 * Get the video details array of the current slide from the slider loop
 */
function get_the_fa_video(){
	
	global $fa_slider;	
	$post = get_current_slide();
	// get slider options and check if titles are visible
	$slider_options = fa_get_slider_options( $fa_slider->ID, 'content_video' );
...
function the_fa_title( $before = '<h2>', $after = '</h2>', $echo = true, $raw = false ){
	...
	// if raw title is to be returned, stop here
	if( $raw ){
		if( $echo ){
			echo $title;
		}
...
function the_fa_content( $before = '', $after = '', $echo = true ){
	...
	}elseif( 'custom' == $slider_options['use'] && strcmp( preg_replace("|([^a-z])|i", '', $options['content'] ) , preg_replace("|([^a-z])|i", '', $post->post_content) ) !== 0 ){
...
function the_fa_read_more( $class = 'fa-read-more', $echo = true ){
	...
	$text 	= false;
	$url 	= '/news-stories/teacher-profiles';
...
function the_fa_play_video( $class = '', $open = 'self', $show_text = true, $echo = true ){
	...
	// get slider options and check if titles are visible
	$video_options 	= fa_get_slider_options( $fa_slider->ID, 'content_video' );
	$link_options 	= fa_get_slider_options( $fa_slider->ID, 'content_play_video' );
	if( !$video_options['show'] || !$link_options['show'] ){
		return;
	}
	
	$options = fa_get_slide_options( $post->ID );
	if( !isset( $options['video']['video_id'] ) || !$options['video']['video_id'] ){
		return;
	}
	
	$data = array();
	if( $open ){
		$targets = array( 'self', 'modal' );
		if( !in_array( $open , $targets) ){
			$open = 'self';
		}
		$data[] = 'data-open_video="' . esc_attr( $open ) . '"';		
	}
	
	$text = esc_attr( $link_options['text'] );
	if( !$show_text ){
		$text = '';
	}
	
	$output = '<a href="#" title="" class="play-video single-link ' . esc_attr( $class ) . '" ' . implode(' ', $data) . '>' . $text . '</a>';
	if( $echo ){
		echo $output;
	}	
	return $output;
...
/**
 * Return slide option of current slide in slides loop
 *
 * @param string $option
 * @return NULL|string
 */

function get_slide_option( $option ){
    $post = get_current_slide();
    if( !$post ){
        return;
    }
...
/**
 * Get slide option for current slide in slides loop
 * 
 * @param string $option - name of option
 * @param string $before - output before option value
 * @param string $after - output after option value
 * @param boolean $echo_empty - echo output if empty( true ) or hide it (false)
 * @param boolean $echo - echo output (true) or hide it (false)
 * @return void|string
 */
function the_slide_option( $option, $before = '', $after = '', $echo_empty = false, $echo = true ){
    $option = get_slide_option( $option );
    if( is_null( $option ) ){
        return;
    }
    
    if( $echo && ( !$echo_empty && !empty( $option ) ) ){
        echo $before . $option . $after;
    }
...
function fa_get_custom_image( $post, $size = array( 'width' => 0, 'height' => 0 ) ){
	...
	if( 'attachment' == $post->post_type ){
		...
		if( !$image_id ){
			...
			if( $fa_slider ){
				$slider_options = fa_get_slider_options( $fa_slider->ID, 'content_image' );
				if( 0 != $slider_options['default_image'] ){
					$image_id = absint( $slider_options['default_image'] );
				}
			}		
			if( !$image_id ){		
				return false;
