...
/**
 * Returns the post status for expired sliders
 */
 ...
 */
function fa_slide_tax(){
	global $fa_pro;
	return $fa_pro->slide_tax();
}
...
/**
 * Checks if slider theme edit preview is on
 */
...
function fa_display_slider( $slider_id, $dynamic_area = false ){
	...
	// Output some debug information if debug is on
...
/**
 * Returns a video URL from a given video ID and a source
 * @param string $video_id
 * @param string $source
 */
function fa_video_url( $video_id, $source ){
	$sources = array(
	...
	if( array_key_exists( $source , $sources) ){
		return sprintf( $sources[ $source ], $video_id );
	}
	return false;	
}
...
function get_the_fa_slide_image_url( $post_id = false, $slider_id = false ){
	...
	if( 'attachment' == $post->post_type ){
		$image_id = $post_id;
	}else{// get the post image ID
		$image_id = get_the_fa_image_id( $post_id );
	}
	
	// if image wasn't found, try to get the one set as default image in slider settings
	if( !$image_id ){
...
/**
 * Display the sliders set on a dynamic area
 * @param string $area - the area ID
 * @param string $before - HTML to be displayed before the area output
 * @param string $after - HTML to be displayed after the area output
 */
function fa_dynamic_area( $area, $before = '', $after = '' ){
