...
class FA_Taxonomies_List_Table extends WP_List_Table {
	...
	function get_views(){
		...
		if( !isset( $_GET['show_all'] ) ){
			...
			$allowed = fa_get_registered_taxonomies();
			$taxonomies = array();
			foreach( $allowed as $post_type => $tax ){
				foreach( $tax as $t ){
		...
		foreach( $taxonomies as $taxonomy ){
			...
			// allow only categories and post tags for regular posts
			if( $tax_obj->_builtin && ( 'category' != $taxonomy && 'post_tag' != $taxonomy ) ){
				continue;
			}	
			
			$class = $this->taxonomy == $taxonomy ? 'current' : '';
			...
			$tax_url = fa_iframe_admin_page_url( 'fa-tax-modal', $args, false );			
		...
		/**
		 * Action that can be used by third party plugins to insert extra views.
		 * For example, this is used by WPML compatibility layer to insert the language switcher.
		 */
		do_action( '_fa_taxonomies_table_views', $this->post_type, $this->taxonomy );
	...
	function display_rows_or_placeholder() {
		...
		$args = wp_parse_args( $this->callback_args, array(
			'page'       => $this->get_pagenum(),
			'number'     => $this->get_items_per_page( 'edit_' . $this->taxonomy . '_per_page' ),
			'search'     => ( !empty( $_REQUEST['s'] ) ? trim( wp_unslash( $_REQUEST['s'] ) ) : '' ),
			'hide_empty' => 0
		) );
	...
	/**
	 * Override parent search_box method to add new input fields
	 * @param string $text
	 * @param int $input_id
	 */
