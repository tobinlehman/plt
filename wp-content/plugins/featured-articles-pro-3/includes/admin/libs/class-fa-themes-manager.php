...
class FA_Themes_Manager{
	...
	/**
	 * Holds the path for the addon themes 
	 * @var string
	 */
	private $addon_themes_path;
	/**
	 * Holds the URL to addon themes folder
	 * @var string
	 */
	private $addon_themes_url;
	/**
	...
	public function __construct(){
		...
		$plugin = 'featured-articles-pro-3-extension-pack/index.php';
		if( is_plugin_active( $plugin ) ){
			$rel_path = 'featured-articles-pro-3-extension-pack/themes';
			$path 	  = wp_normalize_path( path_join( WP_PLUGIN_DIR, $rel_path ) );
			if( is_dir( $path ) ){
				$this->addon_themes_path = $path;
				$this->addon_themes_url = ( is_ssl() ? str_replace('http://', 'https://', WP_PLUGIN_URL) : WP_PLUGIN_URL ).'/'.$rel_path;
			}
		}		
	}	
	...
	public function get_themes(){
		...
		$addon_themes = $this->read_themes( 'addon' );
		$extra_themes = $this->read_themes( 'extra' );		
		$this->themes = array_merge( $this->themes, $addon_themes, $extra_themes );
	...
	private function read_themes( $which = 'default' ){
		...
		switch( $which ){
			case 'default':
				$base_path 	= $this->default_themes_path;
				$base_url 	= $this->default_themes_url;
			break;
			case 'addon':
				$base_path 	= $this->addon_themes_path;
				$base_url 	= $this->addon_themes_url;
			break;
			case 'extra':
				$base_path 	= $this->extra_themes_path;
				$base_url 	= $this->extra_themes_url;
			break;	
		}
