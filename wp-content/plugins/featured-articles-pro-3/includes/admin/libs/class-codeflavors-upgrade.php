...
if( !class_exists('CodeFlavors_Upgrade') ){
	...
	class CodeFlavors_Upgrade{
		...
		*/
		public function __construct( $update_url, $plugin, $slug = '', $request_params = array() ){
			$this->url = $update_url;
			...
			add_filter( 'pre_set_site_transient_' . 'update_plugins', array( $this, 'set_update_transient' ), 10/*, 2*/ );
		...
		public function set_update_transient( $value/*, $transient*/ ){
			if( isset( $value->checked[ $this->plugin_slug ] ) || isset( $value->checked[ $this->plugin ] ) ){
				$slug = isset( $value->checked[ $this->plugin_slug ] ) ? $this->plugin_slug : $this->plugin;
				$version = $value->checked[ $slug ];
				/**
...
if( !class_exists( 'CodeFlavors_Update_Details' ) ){
	/**
	 ...
	 *
	 */
	class CodeFlavors_Update_Details{
		/**
		 * Script version
		 * @var string - version
		 */
		...
		*/
		public function __construct( $changelog_url, $plugin, $slug = '', $request_params = array()  ){
		...
		private function request( $args ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error('codeflavors_plugins_api_failed', __( 'An unexpected error occurred. Something may be wrong with CodeFlavors.com or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://www.codeflavors.com/codeflavors-forums/">support forums</a>.' ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( ! is_object( $res ) && ! is_array( $res ) )
					$res = new WP_Error('codeflavors_plugins_api_failed', __( 'An unexpected error occurred. Something may be wrong with CodeFlavors.com or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://www.codeflavors.com/codeflavors-forums/">support forums</a>.' ), wp_remote_retrieve_body( $request ) );
			}
			
			return $res;
		}
	}// class CodeFlavors_Upgrade_Details
}//end if