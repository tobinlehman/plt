	...
	public function __construct( $post_id = false ){
		...
		$this->defaults = array(
			'slides' => array(
				...
				'type' => 'post',
				/**
				 * For slides made of posts, contains the allowed post types to be displayed
				 * into the slider
				 */
				...
				'posts' 		=> array(),
				/**
				 * Array of manually selected image ids to be used as slides.
				 * Used when slides['type'] = 'image'
				 * 
				 * Image ids are stored as: image_order => image_id
				 * ie: 0=>image_id, 1=>other_image_id ...
				 * 
				 */
				'images'		=> array()
			),
			...
			'content_image' => array(
				...
				'default_image'	=> 0,			// a default image to be displayed on slides without image
			...
			'content_video' => array(
				'show'	=> true, // show videos if any attached to slider 
			),
			...
			// the play video link
			'content_play_video' => array(
				'show' => true,
				'text' => __('Play video', 'fapro'),
			),
			// post date option
			'content_date' => array(
			...
			// slider post type specific functionality
			'slider' => array(
				'expires' 	=> '0000-00-00 00:00:00', // slider expiration date
				'cached'	=> false, // if true, slider will be cahed if plugin setting for caching is on
			),
...
class FA_Slide_Options extends FA_Options{
	...
	public function __construct( $post_id = false ){
		...
		$this->defaults = array(
			...
			'url' 			=> '', // read more url
			'link_target' 	=> '_self', // read more link target
			...
			'video' => array(
				...
				'play'		 => false, // play when slide is active
				...
				'autohide' 		 => false, // autohide controls when player starts playing
		...
		/**
		 * Filter that allows extending of default slide options
		 * @var array
		 */
		$extend_defaults = apply_filters( 'fa_slide_options_defaults' , array() );
	...
	public function get_option( $post_id = false ){
		...
		foreach( $options as $k => $v ){
			if( 'title' == $k || 'content' == $k ){
				...
				// if title isn't set on meta or post type is custom slide, show the post title and content
				if( empty( $v ) || fa_post_type_slide() == $post['post_type'] ){
...
class FA_Plugin_Options extends FA_Options {
	...
	public function __construct(){
		...
		$this->defaults = array(
			/**
			 * Stores the category/post ids and the slider ids that should
			 * be displayed into those pages 
			 */
			/*
			'display' => array(
				'categories' 	=> array(), // array is structured as: array( tag_id => array( slider_id => slider_id, ... ) )
				'pages'			=> array(), // array is structured as: array( page_id => array( slider_id => slider_id, ... ) )
				'homepage'		=> array() // array is structured as: array( slider_id => slider_id, ... )
			),
			*/
			/**
			...
			'settings' => array(
				...
				'custom_posts'		 => array(), // allow custom post types in slideshows
				'post_slide_edit'	 => true, // allow slide edit meta panel to be displayed when editing posts
				'themes_dir' 		 => '', // extra slideshow themes relative folder path (must be inside wp-content folder)
				'cache'				 => false, // sliders will be cached for 12H
			...
			/**
			 * Defaults to be loaded when creating a new color scheme for an
			 * existing slideshow theme.
			 */
			'theme_editor' => array(
				'slider_id' 	=> 0,
				'area_id' 		=> 'loop_start'
		...
		/**
		 * Filter license to check a few things
		 */
		add_filter('fa-set-options_license', array( $this, 'update_license' ), 1, 4);
		// run action after saving the license
		add_action('fa-updated-options_license', array( $this, 'validate_license' ), 1, 4);
		
		/**
		 * Filter hooks to sanitize properly
		 */		
		add_filter('fa-set-options_hooks', array( $this, 'update_hooks' ), 1, 4);
	 ...
	 * Ran by filter fa-set-options_license set in class constructor.
	 * 
	 * @param array $processed - values processed by default by the update function
	 * @param array $values - the values that must be saved
	 * @param string $key - the key in plugin options array that needs to be saved
	 * @param array $option - the complete option from database
	 */
	public function update_license( $processed, $values, $key, $option ){
		if( isset( $values['license_key'] ) ){
			if( empty( $values['license_key'] ) ){
				return $processed['license_key'] = '';
			}else if( 32 != strlen( $values['license_key'] ) ){
				$error = new WP_Error();
				$error->add('fapro-invalid-license', __('The license key is not valid.', 'fapro'));
				return $error;
			}else{
				// do the registration
				$registration = register_fa_license( $values['license_key'] );
				if( is_wp_error( $registration ) ){
					return $registration;
				}else if( 400 == wp_remote_retrieve_response_code( $registration ) ){
					// retrieve the error message
					$message = wp_remote_retrieve_body( $registration );
					return new WP_Error( 'fa_activation_error_message', $message );
				}
				// add activation date to the details
				$processed['activation_date'] = current_time('mysql');				
			}	
		}
		return $processed;
	}
	...
	/**
	 * Callback after updating the license key.
	 * 
	 * @param array $processed - values processed by default by the update function
	 * @param array $values - the values that must be saved
	 * @param string $key - the key in plugin options array that needs to be saved
	 * @param array $option - the complete option from database
	 */
	public function validate_license( $processed, $values, $key, $option ){
		fa_check_details( true );
	}
	
	/**
	 * Save the custom dynamic areas.
	 * Ran by filter fa-set-options_hooks set in class constructor.
	 * 
	 * @param array $processed - values processed by default by the update function
	 * @param array $values - the values that must be saved
	 * @param string $key - the key in plugin options array that needs to be saved
	 * @param array $option - the complete option from database
	 */
	public function update_hooks( $processed, $values, $key, $option ){
		// get the default hooks; these can't be modified
		$defaults = $this->defaults['hooks'];
		if( !is_array( $values ) ){
			return $defaults;
		}
		// process the values
		foreach( $values as $hook_id => $hook_details ){
			if( array_key_exists( $hook_id, $defaults ) ){
				if( isset( $hook_details['sliders'] ) && is_array( $hook_details['sliders'] ) ){
					$defaults[ $hook_id ]['sliders'] = $hook_details['sliders'];
				}				
				continue;
			}
			
			$hook_id 		= fa_sanitize_hook_id( $hook_id );
			$name 			= sanitize_text_field( $hook_details['name'] );
			$description 	= sanitize_text_field( $hook_details['description'] );
			$sliders		= isset( $hook_details['sliders'] ) && is_array( $hook_details['sliders'] ) ? $hook_details['sliders'] : array();
			
			$defaults[ $hook_id ] = array(
				'name' 			=> $name,
	...
	public function get_option( $key = false ){		
		...
		if( array_key_exists( $key, $option ) ){
			/**
			 * Filter that can manipulate the returned plugin option
			 * 
			 * @param $option: the returned option
			 * @param $key : options key in plugin options to be updated
			 * @param $full_option: all options
			 * 
			 * @var array
			 */
			$o = apply_filters( 'fa-get-options-' . $key, $option[ $key ] );			
			
			return wp_parse_args( $o, $this->defaults[ $key ]);
		}else{
