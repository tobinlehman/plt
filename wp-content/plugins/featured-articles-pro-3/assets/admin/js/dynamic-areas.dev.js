...
faSliders = {
	...
	init : function() {
		...
		$('.fa-area-code-label').click(function(){
			$(this).next('.fa-area-desc').toggle();
		})
		...
		$(document.body).bind('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				faSliders.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				faSliders.save( target.closest('div.widget'), 1, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-close') ) {
				faSliders.close( target.closest('div.widget') );
				e.preventDefault();
			}
		});
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				if ( $widget.hasClass('deleting') ) {
					faSliders.save( $widget, 1, 0, 1 ); // delete widget
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					faSliders.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			action: fa_dynamic_areas.assign_action,
			areas: []
		};
		data[ fa_dynamic_areas.assign_nonce.name ] = fa_dynamic_areas.assign_nonce.nonce;
		...
		$('div.widgets-sortables').each( function() {
			if ( $(this).sortable ) {
				data['areas[' + $(this).attr('id') + ']'] = $(this).sortable('toArray').join(',');
			}
		});
...
$(document).ready( function(){ 
	...
	$('.fa-area-action.action-delete').click(function(e){
		if( !confirm( fa_dynamic_areas.messages.delete_confirm ) ){
			e.preventDefault();
		}		
	})
