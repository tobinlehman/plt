		...
		var center_image = function( img ){
       	...
       	var navigation = function(){
       		...
       		if( options.nav_elem ){
       			...
       			// change mouse event if neccessary
           		var nEvent = nav_event;
       			if( !touch && 'click' != options.event ){
       				nEvent = 'mouseenter';
           		}
       			
       			
       			$.each( navs, function( i ){
       				$(this).bind( nEvent, function(e){
       					e.preventDefault();
       					if( options.click_stop ){
           					stop_auto_slide();
           				}
						...
						open : function( event, ui ){
							...
							if( !$(slide).data('player') ){	
								var player = $(this).fa_video({
									...
									onPlay: function( status ){
										options.video_status.call( self, status );
										set_video_status( status );
									},
									onPause: function( status ){
										options.video_status.call( self, status );
										set_video_status( status );										
									},
       	...
       	var apply_slide_effect = function(){
       		...
       		if( !e || !get_effect( e ) ){
       			// effect not found, default to fade
       			e = 'fade';
       		}
       	 ...
       	 * Effects
       	 */
       	var get_effect = function( effect, all ){
       		var effects = {
   				squares : {
   					cols 		: 8,
   					rows		: 4,
   				...
   				horizontal_slices : {
   					cols	: 1,
   					rows	: 25,
   					animation : { x : - ( $(this).width() + 200 ), opacity:0 },
   					delayed : true,
   					reversed : true
   				},
   				vertical_slices : {
   					cols	: 25,
   					rows	: 1,
   					animation : { y : ( $(this).height()+200 ), opacity:0 },
   					delayed : true,
   					reversed : false
   				},  
   				swirl : {
   					cols	: 1,
   					rows	: 1,
   					animation : { rotate:1000, opacity:0, scale:0 },
   					delayed : true,
   					reversed : false
   				},
   				out_right : {
