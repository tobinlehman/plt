...
;(function($){
	...
	$.fn.CFCarousel = function(options){
		...
		var options 		= $.extend({}, defaultOptions, options),
			self 			= this,
			outerContainer 	= $(this).find('.fa_carousel_inside'),
			container		= $(this).find('.fa_carousel_items'), // items container
		...
		var setSideNavs = function(){
			...
			$(self).mouseenter(function(){
				$(navBack).stop().animate({'opacity':1, 'left' : -15},{queue:false, duration:200});
				$(navFwd).stop().animate({'opacity':1, 'right' : -15},{queue:false, duration:200});
			}).mouseleave(function(){
