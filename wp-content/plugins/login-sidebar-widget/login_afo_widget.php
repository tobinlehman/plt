...
class login_wid extends WP_Widget {
	...
	public function add_extra_links(){
		...
		if($login_afo_forgot_pass_link){
			echo '<a href="'. esc_url( get_permalink($login_afo_forgot_pass_link) ).'" ' . do_action( 'lwws_lost_password_link_a_tag' ) . '>'.__('Lost Password?','login-sidebar-widget').'</a>';
		}
		...
		if($login_afo_register_link){
			echo '<a href="'. esc_url( get_permalink($login_afo_register_link) ) .'" ' . do_action( 'lwws_register_link_a_tag' ) . '>'.__('Register','login-sidebar-widget').'</a>';
		}
	...
	public function loginForm( $wid_id = '' ){
		...
		<form name="login" id="login" method="post" action="" autocomplete="off" <?php echo do_action( 'lwws_login_form_tag' );?>>
		...
		<div class="log-form-group">
			...
			<input type="text" name="userusername" required <?php echo do_action( 'lwws_userusername_field' );?>/>
		</div>
		<div class="log-form-group">
			<label for="password"><?php _e('Password','login-sidebar-widget');?> </label>
			<input type="password" name="userpassword" required <?php echo do_action( 'lwws_userpassword_field' );?>/>
		</div>
		...
		<div class="log-form-group"><input name="login" type="submit" value="<?php _e('Login','login-sidebar-widget');?>" <?php echo do_action( 'lwws_login_form_submit_tag' );?>/></div>
		...
		if($link_in_username){
			$link_with_username = '<a href="' . esc_url( get_permalink($link_in_username) ) . '" ' . do_action( 'lwws_username_link_a_tag' ) . '>' . apply_filters( 'lwws_welcome_text', __('Howdy,','login-sidebar-widget') ) . ' ' . $current_user->display_name . '</a>';
		} else {
			$link_with_username = apply_filters( 'lwws_welcome_text', __('Howdy,','login-sidebar-widget') ) . ' ' . $current_user->display_name;
		}
		?>
        <div class="logged-in"><?php echo $link_with_username;?> | <a href="<?php echo wp_logout_url( apply_filters( 'lwws_logout_redirect', $logout_redirect_page, $current_user->ID ) ); ?>" title="<?php _e('Logout','login-sidebar-widget');?>" <?php echo do_action( 'lwws_logout_link_a_tag' ); ?>><?php _e('Logout','login-sidebar-widget');?></a></div>
		<?php 
		}
	}
