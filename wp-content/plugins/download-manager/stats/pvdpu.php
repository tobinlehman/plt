...
$files = $wpdb->get_results("select s.pid,f.post_title  from {$wpdb->prefix}ahm_download_stats s,{$wpdb->prefix}posts f where $uid f.ID=s.pid group by pid",ARRAY_A);
...
foreach($files as $file){
    ...
    foreach($dates as $date){
          ...
          echo "<td class='info' title='".$fstats[$file['pid']][$date['dt']]." downloads' align=center><a href='edit.php?post_type=wpdmpro&page=wpdm-stats&type=pvdaud&pid={$file['pid']}&y={$date['year']}&m={$date['month']}&d={$date['day']}'>".$fstats[$file['pid']][$date['dt']]."</a></td>";
          else
