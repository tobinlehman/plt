...
function wpmp_dir_browser_metabox($post){
    ?>
    <input style="width: 80%;border-radius: 3px" type="text" id="srvdir" value="<?php echo get_post_meta($post->ID,'__wpdm_package_dir', true); ?>" name="file[package_dir]" />
    <a style="margin-top:1.5px;padding:2px 7px;" href="admin.php?page=file-manager&task=wpdm_dir_browser" class="thickbox button-secondary"><img src="<?php echo WPDM_BASE_URL.'images/folder_open.png'; ?>" title="<?php echo __('Open Dir Browser','wpdmpro'); ?>" /></a>
...
function wpdm_get_files($dir){
    $dir = rtrim($dir,"/")."/";
    $tmpfiles = array_diff( scandir( $dir ), Array( ".", ".." ) );
    $files = array();
    foreach($tmpfiles as $file){
        if( is_dir($dir.$file) ) $files = array_merge($files,wpdm_get_files($dir.$file));
        else
        $files[] = $dir.$file;
    }
    return $files;
