<?php
function wpdm_additional_preview_images($id){

   
?>

<a id="add-prev-img" href="#"><img align="right" src="<?php echo plugins_url('/download-manager/images/add-image.gif'); ?>" /></a>
<div id="adpcon">
<?php

    $mpvs = get_post_meta(get_the_ID(),'__wpdm_additional_previews', true);
    ...
    if(is_array($mpvs)){
        ...
        foreach($mpvs as $mpv){
            ...
            ?>
             ...
             <img src='<?php echo wpdm_dynamic_thumb($mpv, array(50,50)); ?>'/>
  ...
  jQuery('body').on('click', '#add-prev-img', function( event ){
    ...
    file_frame.on( 'select', function() {
      ...
      var newDate = new Date;
        ...
        jQuery('#adpcon').append("<div id='"+ID+"' style='display:none;float:left;padding:5px;' class='adp'><input type='hidden' id='in_"+ID+"' name='file[additional_previews][]' value='"+imgurl+"' /><nobr><b><img style='position:absolute;z-index:9999;cursor:pointer;' id='del_"+ID+"' src='<?php echo plugins_url(); ?>/download-manager/images/minus.png' rel='del' align=left /><img style='height:50px' src='"+imgurl+"'/></b></nobr><div style='clear:both'></div></div>");
        jQuery('#'+ID).fadeIn();
