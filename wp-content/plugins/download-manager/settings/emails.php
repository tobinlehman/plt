...
function wpdm_common_actions()
{
    ...
    $args = array(
        ...
        'query_var' => true,
        'rewrite' => array('slug' => get_option('__wpdm_purl_base', 'download'), 'with_front' => get_option('__wpdm_purl_with_front', false)), //get_option('__wpdm_purl_base','download')
        'capability_type' => 'post',
        'has_archive' => get_option('__wpdm_has_archive', false),
        'hierarchical' => false,
        'menu_position' => null,
        'taxonomies' => array('post_tag'),
        'menu_icon' => 'dashicons-download',
        'exclude_from_search' => get_option('__wpdm_exclude_from_search', false),
    ...
    $args = array(
        ...
        'query_var' => true,
        'rewrite' => array('slug' => get_option('__wpdm_curl_base', 'download-category')),
    );
...
function DownloadLink(&$package, $embed = 0, $extras = array())
{
    ...
    if (isset($package['expire_date']) && $package['expire_date'] != "" && strtotime($package['expire_date']) < time()) {
        $package['download_url'] = '#';
        $package['link_label'] = __('Download was expired on', 'wpdmpro') . " " . date(get_option('date_format')." h:i A", strtotime($package['expire_date']));
    ...
    if (isset($package['publish_date']) && $package['publish_date'] !='' && strtotime($package['publish_date']) > time()) {
        $package['download_url'] = '#';
        $package['link_label'] = __('Download will be available from ', 'wpdmpro') . " " . date(get_option('date_format')." h:i A", strtotime($package['publish_date']));
        $package['download_link'] = "<a href='#'>{$package['link_label']}</a>";
        return "<div class='alert alert-warning'><b>" . __('Download:', 'wpdmpro') . "</b><br/>{$package['link_label']}</div>";
    }
  ...
  <div class="panel-body">
        ...
        if ($lock === 'locked') {
            ...
            if ($embed == 1)
                ...
                $adata = '<a href="#pkg_' . $package['ID'] . '" data-title="'.__('Download','wpdmpro').' ' . $package['title'] . '" class="wpdm-download-link wpdm-download-locked ' . $popstyle . ' ' . $btnclass . '"><i class=\'' . $wpdm_download_lock_icon . '\'></i>' . $package['link_label'] . '</a>';
...
function wpdm_email_lock_form($package)
{
  ...
  <div class="panel-body">
        ...
        jQuery("#wpdmdlf_' . $unqid . '_' . $package['ID'] . '").submit(function(){
            ...
            jQuery(this).removeClass("wpdm_submit").addClass("wpdm_submit_wait");
            jQuery(this).ajaxSubmit({success:function(res){
                jQuery("#wpdmdlf_' . $unqid . '_' . $package['ID'] . '").hide();
  ...
  <div class="panel-body" id="in_{$tmpid}">
                ...
                twttr.ready(function (twttr) {
                    ...
                    twttr.events.bind('tweet', function(event) {
                        ...
                        jQuery.post("{$home}",{id:{$package['ID']},dataType:'json',execute:'wpdm_getlink',force:'$force',social:'t',action:'wpdm_ajax_call'},function(res){                                                                
                            if(res.downloadurl!=""&&res.downloadurl!=undefined) {
                            location.href=res.downloadurl;
  ...
  <div class="panel-body" id="ll_{$tmpid}">
                ...
                <script type="text/javascript">
                  ...
                  function wpdm_linkedin_unlock_$lis(){                         
                        jQuery.cookie('unlocked_{$package['ID']}',1); 
                        jQuery.post("{$home}",{id:{$package['ID']},dataType:'json',execute:'wpdm_getlink',force:'$force',social:'l',action:'wpdm_ajax_call'},function(res){                                                                
                            if(res.downloadurl!=""&&res.downloadurl!=undefined) {
                            location.href=res.downloadurl;
...
<div class="g-plusone" data-size="small" data-callback="oki" data-href="http://google.com"></div>
                ...
                <script type="text/javascript">
                  ...
                  console.log(plusone);
                        ...
                        jQuery.cookie('unlocked_{$package['ID']}',1); 
                        jQuery.post("{$home}",{id:{$package['ID']},dataType:'json',execute:'wpdm_getlink',force:'$force',social:'g',action:'wpdm_ajax_call'},function(res){                                                                
                            if(res.downloadurl!=""&&res.downloadurl!=undefined) {
                            location.href=res.downloadurl;
        ...
        <tr>
            <td>
                ...
                <label><input type="radio" id="idl" name="file[email_lock_idl]"
                              value="0" <?php if ($idl != 1) echo 'checked=checked'; ?>> Mail Download Link</label>
                <label><input type="radio" id="idl" name="file[email_lock_idl]"
                              value="1" <?php if ($idl == 1) echo 'checked=checked'; ?> > Downlaod Instantly</label>
                <br/>
            </td>
        </tr>
    </table>
...
function wpdm_dashboard_widget_function()
{
    ...
    <div class="w3eden">
        <ul class="nav nav-tabs" id="myTab">
            <li class="active"><a href="#home">Summery</a></li>
            <li><a href="#social">Social</a></li>
            <li><a href="#messages">Messages</a></li>
            <li><a href="#settings">News Updates</a></li>
        </ul>
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane" id="social">
                <table class="table table-bordered table-striped" style="margin-bottom: 0px;width:100%">
                    ...
                    <tr>
                        <td>Total Goggle +1</td>
                        <th><?php echo get_option('wpdm_gplus1s', 0); ?></th>
                    </tr>
                    <tr>
                        <td>Total LinkedIn Shares</td>
            ...
            <div class="tab-pane" id="settings">
                <iframe src="http://wpdownloadmanager.com/notice.php?wpdmvarsion=<?php echo WPDM_Version; ?>"
...
function wpdm_similar_packages($package_id = null, $count = 5)
{
    ...
    if(count($posts)==0) $html = "<div class='col-md-12'><div class='alert alert-info'>No related download found!</div> </div>";
    ...
    if(is_archive() && get_post_type()=='wpdmpro'){
        ...
        if(get_option('__wpdm_cpage_style')=='basic'){
            ...
            if(isset($cpageinfo['version']))
                $info .="<li><i class='fa fa-history'></i> &nbsp;Version {$data['version']}</li>";

            if(isset($cpageinfo['view_count']))
                $info .="<li><i class='fa fa-eye'></i>  &nbsp;{$data['view_count']} View(s)</li>";

            if(isset($cpageinfo['download_count']))
                $info .="<li><i class='fa fa-download'></i>  &nbsp;{$data['download_count']} Download(s)</li>";
        ...
        if(get_option('__wpdm_cpage_style')=='ltpl'){
            return "<div class='w3eden'>".FetchTemplate(get_option('__wpdm_cpage_template'), $data).get_option('__wpdm_cpage_template')."</div>";
        }
