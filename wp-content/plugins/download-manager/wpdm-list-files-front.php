...
<div class="tablenav text-right" style="line-height: 25px;margin-top: 20px;">
<nobr style="line-height: 25px;">
<input type="hidden" name="do" value="search" />
<input type="text" id="sfld" class="form-control input-sm" style="display: inline;width: 140px;" name="q" value="<?php echo $Q; ?>">
<button type="submit" class="btn btn-primary btn-sm" id="doaction">Search</button>
<?php if($Q) { ?>
<a  class="btn btn-danger btn-sm" href='<?php the_permalink(); ?>' >Reset Search</a>
<?php } ?>
...
<!--    <th style="" class="manage-column column-media" id="media" scope="col">Embed Code</th>    -->
    ...
    <tr valign="top" class="alternate author-self status-inherit" id="post-<?php the_ID(); ?>">
                ...
                <td class="media column-media">
                    <a title="Edit" href="<?php echo $burl.$sap; ?>task=edit-package&id=<?php the_ID(); ?>"><?php the_title();?></a>
                </td>
...
<!--
  jQuery(function(){
     ...
     jQuery('.delp').popover({placement:'left', html:true});
      jQuery('.canceldelete').on('click',function(){
